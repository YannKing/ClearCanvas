                ...
                while (_remain > 0)
                {
                        ...
                        && (_sqrs.Count == 0)) // only exit in root message when after stop tag
                    {
                        if (_inGroup2 && tagValue > 0x0002FFFF)
                        {
                            ...
                            {
                                Platform.Log(LogLevel.Debug, "File Meta Info Length, {0}, not equal to actual bytes read in file, {1}, overwriting length.", EndGroupTwo, BytesRead - 4);

                                XTrace.WriteLine($"文件元信息长度，{EndGroupTwo}，不等于文件中读取的实际字节数，{BytesRead - 4}，覆盖长度。");
                    ...
                    if (_vr == null)
                    {
                        ...
                        if (_vr == DicomVr.UNvr)
                        {
                            if (LastTagRead.IsPrivate)
                            {
                                ...
                                {
                                    // Reset the tag with the right VR and a more descriptive name. 使用正确的VR和更具描述性的名称重置标记。
                    ...
                    if (_fragment != null)
                    {
                        ...
                        else
                        {
                            ...
                            XTrace.WriteLine($"在流中遇到意外标记：{LastTagRead.ToString()}");
                            // unexpected tag //意外的标签
                        ...
                        || LastTagRead == DicomTag.SequenceDelimitationItem))
                    {
                        ...
                        if (LastTagRead.Equals(DicomTag.Item))
                        {
                            ...
                            // 特定字符集是继承的，保存它。它将被覆盖如果在序列中遇到标记的新值。
                            ...
                            if (_len != _undefinedLength)
                            {
                                ...
                                if (stat != DicomReadStatus.Success)
                                {
                                    ...
                                    XTrace.WriteLine($"读取序列属性时出现意外的解析错误（{stat}）：{rec.Tag.ToString()}。");
                                    return stat;
                                }
                            }
                        }
