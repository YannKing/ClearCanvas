...
{
    /// <summary>DicomTag类包含特定标记的所有DICOM信息。
    /// 
    /// The DicomTag class contains all DICOM information for a specific tag.
    /// </summary>
    /// <remarks>
    /// <para>使用DicomTag类，如Flyweight模式中所述。
    /// 只应分配一个实例对于每个DICOM标记，该实例将在任何<see cref ="DicomAttributeCollection"/>中共享引用特定标记。
    /// 
    /// The DicomTag class is used as described in the Flyweight pattern.  A single instance should only be allocated
    /// for each DICOM tag, and that instance will be shared in any <see cref="DicomAttributeCollection"/> 
    /// that references the specific tag.
    /// </para>
    /// 
    /// <para>但请注意，非标准DICOM标签（或标签未存储在<see cref ="DicomTagDictionary"/>将具有一个特定的实例，用于在程序集遇到它们时存储它们的信息。
    /// 
    /// Note, however, that non standard DICOM tags (or tags not in stored in the <see cref="DicomTagDictionary"/>
