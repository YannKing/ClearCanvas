...
namespace ClearCanvas.Dicom.Network
{
    ...
    public class RawPDU
    {
        ...
        /// <remarks>
        /// <para>
        /// 假设在编写/创建新PDU时必须使用此构造函数。这个构造函数读取PDU时无效。
        /// It is assumed that this contructor must be used when writing / creating a new PDU.  This constructor will not work when reading a PDU.
        /// </para>
        /// 
        /// <para>
        /// 注意：将此类拆分为写入PDU和写入的类可能是有意义的读取PDU的第二个类。
        /// NOTE:  It might make sense in the future to split this class into one that writes PDUs and 
        /// a second class that reads PDUs.
        /// </para>
        /// </remarks>
        /// 
        /// <param name="type">正在创建PDU类型。 
        /// The PDU type being created.</param>
        /// <param name="max">PDU的最大长度
        /// The maximum length of the PDU</param>
        ...
        private void CheckOffset(int bytes, String name)
        {
            ...
            {
                String msg = String.Format("{0} (offset={1}, bytes={2}, field=\"{3}\") Requested offset out of range!", ToString(), _ms.Position, bytes, name);

                var logTxt = $"{ToString()} (offset={_ms.Position}, bytes={bytes}, field=\"{name}\") 请求偏移超出范围!";
                XTrace.WriteLine(logTxt);

                var throwEx = new DicomNetworkException(msg);
                XTrace.WriteException(throwEx);
                throw throwEx;
            }
        }
    ...
    /// <summary>IPDU接口定义
    /// Yann 2019年1月3日 13:18:11</summary>
    ...
    public class AAssociateRQ : IPDU
    {
        ...
        public RawPDU Write()
        {
            ...
            XTrace.WriteLine($"{System.Reflection.MethodBase.GetCurrentMethod().DeclaringType?.FullName} " +
