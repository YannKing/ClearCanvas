...
namespace ClearCanvas.Dicom.Network.Scu
{
    ...
    // TODO CR (23 Jan 2014): 如果实际上在不重写OnReceiveResponse的情况下永远不能使用该类，则将此类设为抽象
    ...
    /// <remarks>
    /// 被覆盖的三个主要方法是
    /// <see cref ="SetPresentationContexts"/>，
    /// <see cref ="OnReceiveAssociateAccept"/>，
    /// 和<see cref ="OnReceiveAssociateAccept"/>。
    /// 
    /// The three main methods that shouold be overwritten are 
    /// <see cref="SetPresentationContexts"/>, 
    /// <see cref="OnReceiveAssociateAccept"/>, 
    /// and <see cref="OnReceiveAssociateAccept"/>.
    /// 
    /// <para>
    /// 注意，内置于此类中，因此所有Scu类都可以使用<see cref ="Timeout"/>属性和<see cref ="Cancel"/>方法。
    /// 
    /// Note, built into this class, so all Scu classes can use the <see cref="Timeout"/> property and the <see cref="Cancel"/> method. 
    /// </para>
    /// </remarks>
    public class ScuBase : IDicomClientHandler, IDisposable
    {
        ...
        protected void Connect()
        {
            try
            {
                ...
                AssociationParameters = new ClientAssociationParameters(ClientAETitle, RemoteAE, RemoteHost, RemotePort);
            ...
            catch (Exception ex)
            {
                ...
                XTrace.WriteLine($"尝试连接到RemoteHost {RemoteAE}（{RemoteHost}：{RemotePort}）的异常");
        ...
        protected void Connect(string clientAETitle, string remoteAE, string remoteHost, int remotePort)
        {
            if (LogInformation)
            {
                Platform.Log(LogLevel.Info, "Preparing to connect to AE {0} on host {1} on port {2}", remoteAE, remoteHost, remotePort);
                XTrace.WriteLine($"Preparing to connect to AE {remoteAE} on host {remoteHost} on port {remotePort}");
                XTrace.WriteLine($"准备链接到 AE {remoteAE} 在主机：{remoteHost} 端口：{remotePort}");
            }
            try
            {
            ...
            catch (Exception e)
            {
                ...
                XTrace.WriteLine($"尝试连接到端口{remotePort}上主机{remoteHost}上的Remote AE {remoteAE}时发生意外异常");
                XTrace.WriteException(e);
                throw;
            }
        }

        /// <summary>检查取消了。如果操作被取消，则抛出<see cref ="OperationCanceledException"/>。
        /// 
