    ...
    public abstract class CommandProcessor : IDisposable
    {
        ...
        public bool Execute()
        {
            while (_queue.Count > 0)
            {
                ...
                catch (Exception e)
                {
                    if (command.RequiresRollback)
                    {
                        ...
                        XTrace.WriteLine($"执行命令时出现意外错误：{command.Description}");
                    ...
                    Platform.Log(LogLevel.Warn, e, "Unexpected exception on command {0} that doesn't require rollback", command.Description);

                    XTrace.WriteLine($"命令{command.Description}上的意外异常不需要回滚");
                    XTrace.WriteException(e);

                    _stack.Pop(); // Pop it off the stack, since it failed.//将它从堆栈中弹出，因为它失败了。
                }
            }
            ...
            catch (Exception e)
            {
                ...
                XTrace.WriteLine($"提交更新到上下文时出现意外错误。滚回来。");
                XTrace.WriteException(e);
        ...
        public bool ExecuteSubCommand(IAggregateCommand baseCommand, ICommand subCommand)
        {
            ...
            catch (Exception e)
            {
                if (subCommand.RequiresRollback)
                {
                    ...
                    XTrace.WriteLine($"执行命令时出现意外错误：{subCommand.Description}");
                    XTrace.WriteException(e);
                ...
                Platform.Log(LogLevel.Warn, e, "Unexpected exception on command {0} that doesn't require rollback", subCommand.Description);
                XTrace.WriteLine($"命令{subCommand.Description}上的意外异常不需要回滚");
                XTrace.WriteException(e);
        ...
        public void Rollback()
        {
            ...
            while (_stack.Count > 0)
            {
                ...
                catch (Exception e)
                {
                    ...
                    XTrace.WriteLine($"意外异常回滚命令{command.Description}");
                    XTrace.WriteException(e);
                }
        ...
        protected static void RollbackAggregateCommand(IAggregateCommand command)
        {
            while (command.AggregateCommands.Count > 0)
            {
                ...
                catch (Exception e)
                {
                    ...
                    XTrace.WriteLine($"意外异常回滚子命令{subCommand.Description}");
                    XTrace.WriteException(e);
                }
