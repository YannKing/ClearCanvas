...
namespace ClearCanvas.Desktop
{
    ...
    /// <summary>定义一个扩展点，用于提供<see cref ="IGuiToolkit"/>的实现。</summary>
    /// <remarks>该应用程序需要一个扩展点。</remarks>
    [ExtensionPoint]
    public sealed class GuiToolkitExtensionPoint : ExtensionPoint<IGuiToolkit> { }

    /// <summary>定义一个扩展点，用于提供<see cref ="ISessionManager"/>的可选实现。</summary>
    /// <remarks>如果找到，框架将使用此点的一个扩展，但不需要扩展。</remarks>
    [ExtensionPoint]
    public sealed class SessionManagerExtensionPoint : ExtensionPoint<ISessionManager> { }

    /// <summary>为应用程序的视图定义扩展点。</summary>
    /// <remarks>需要一个扩展名，否则应用程序将无法运行。</remarks>
    [ExtensionPoint]
    public sealed class ApplicationViewExtensionPoint : ExtensionPoint<IApplicationView> { }

    /// <summary>扩展工具的工具上下文界面<see cref ="ApplicationToolExtensionPoint"/>。 </summary>
    public interface IApplicationToolContext : IToolContext { }

    /// <summary>定义应用程序工具的扩展点，这是应用程序的全局工具。</summary>
    /// <remarks>
    /// 应用程序工具对应用程序是全局的。应用程序工具只实例化一次。
    /// 应用程序工具无法执行操作，因为它们与任何UI实体都没有关联。
    /// 扩展应该期望接收类型<see cref ="IApplicationToolContext"/>的工具上下文。
    /// 
    ...
    public class Application : IApplicationRoot
    {
        ...
        /// <summary>获取<see cref ="Application"/>对象的单例实例。</summary>
        ...
        public static event EventHandler<SessionStatusChangedEventArgs> SessionStatusChanged
        {
            add
            {
                SessionManager.Current.SessionStatusChanged += value;
            }
            remove
            {
                SessionManager.Current.SessionStatusChanged -= value;
        ...
        public static string GuiToolkitID
        {
            get
            {
                return _instance != null && _instance.GuiToolkit != null ? _instance.GuiToolkit.ToolkitID : null;
            }
        }
        ...
        public static string Name
        {
            get
            {
                return _instance.ApplicationName;
            }
        }
        ...
        public static Version Version
        {
            get
            {
                return _instance.ApplicationVersion;
            }
        }
        ...
        public static DesktopWindow ActiveDesktopWindow
        {
            get
            {
                return DesktopWindows.ActiveWindow;
            }
        }
        ...
        /// <summary>尝试关闭所有打开的桌面窗口并终止应用程序。
        /// Attempts to close all open desktop windows and terminate the application.
        /// </summary>
        /// <remarks>退出请求无法保证成功。具体来说，如果是，它将失败开放工作空间需要用户交互以便关闭，在这种情况下用户可以取消操作。
        /// 该请求也可以由处理程序以编程方式取消<see cref ="Quitting"/>事件。
        ...
        /// </remarks>
        /// <returns>如果应用程序成功退出则为True，否则为false。
        /// True if the application successfully quits, or false if it does not.</returns>
        ...
        /// <summary>强制关闭所有打开的桌面窗口并终止应用程序。
        /// Forcibly closes all open desktop windows and terminates the application.
        /// </summary>
        /// <remarks>该调用将强制终止应用程序而不允许桌面组件取消操作。要允许桌面组件取消操作，考虑调用<see cref ="Quit"/>。
        ...
        /// <summary>使当前会话无效，这通常需要用户重新进行身份验证以继续使用该应用程序。
        ...
        /// <summary>通过运行GUI工具包并创建应用程序视图来实现启动桌面的逻辑。 </summary>
        ...
        private void Quit(bool force)
        {
            if (!_initialized && !force)
                throw new InvalidOperationException("在完全初始化应用程序之前，无法调用此方法");
            ...
            else
            {
                try
                {
                    //是否实际在UI线程上执行是无关紧要的，  因为如果没有，那么应用已经退出了！
        ...
        /// <summary>实现终止桌面的逻辑，包括关闭所有窗口并终止会话。 </summary>
        /// <returns>如果应用程序真的要终止则为True，否则为false。</returns>
