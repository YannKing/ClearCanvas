...
namespace ClearCanvas.ImageViewer.Explorer.Dicom
{
	...
	public class StudyBrowserComponent : ApplicationComponent, IStudyBrowserComponent
	{
		...
		private static List<StudyEntry> GetStudyEntries(IDicomServiceNode server, StudyRootStudyIdentifier criteria)
		{
            ...
            // N.B。：我们必须以这种方式处理搜索条件中的多种模态，因为DICOM不允许对模态进行多值搜索（如果您认为这样做，请仔细重新阅读标准！）
            //规范化模态列表（没有重复项，只有在没有其他模态时才为空字符串）
			...
			using (var bridge = new StudyStoreBridge(storeQuery))
			{
				...
				foreach (var modality in modalities)
				{
					...
					var modalityResults = bridge.GetStudyEntries(modalityCriteria).ToList(); // 'ToList'在这里和现在执行查询

                    if ((modality == string.Empty)
					    || (modalityResults.Any() && modalityResults.All(s => s.Study.ModalitiesInStudy.All(string.IsNullOrEmpty))) //如果sequence为空，'All'返回true，所以确保它不是空的！, 'All' returns true if sequence is empty, so make sure it's not empty!
                        || (modalityResults.Any(s => !s.Study.ModalitiesInStudy.Any(m => modality.Equals(m, StringComparison.InvariantCultureIgnoreCase)))))
					{
                        ...
                        //如果在查询中未过滤模态
                        //或者如果所有结果（非空）都不返回模态（即服务器不支持查询或返回此可选标记）
                        //或者如果任何结果与查询的模态不匹配（即服务器不支持查询此可选标记）
                        //那么这些都是我们将获得的独特结果，我们可以跳过其他方式
