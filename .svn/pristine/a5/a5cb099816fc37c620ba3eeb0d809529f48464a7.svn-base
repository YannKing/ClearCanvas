...
namespace ClearCanvas.Dicom.Utilities
{
    ...
    public sealed class DateRangeHelper
    {
        ...
        /// <summary>将解析遵循dicom日期格式的日期范围，将日期返回为<see cref ="DateTime"/>对象。
        /// 
        /// Will parse a date range adhering to the dicom date format, returning the dates as <see cref="DateTime"/> objects.
        /// </summary>
        /// 
        /// <param name="dateRange">要解析的字符串
        /// the string to be parsed</param>
        /// 
        /// <param name="fromDate">“起始日期”，或null
        /// the "from date", or null</param>
        /// 
        /// <param name="toDate">“截止日期”或为空
        /// the "to date" or null</param>
        /// 
        /// <param name="isRange">输入值是否实际上是一个范围。如果不是，则应采用“起始日期”值是一个确切的值，而不是范围，取决于应用程序。
        /// whether or not the input value was actually a range.  If not, then the "from date" value should be taken 
        /// to be an exact value, not a range, depending on the application.</param>
        /// 
        /// <exception cref="InvalidOperationException">如果输入范围格式不正确
        /// if the input range is poorly formatted</exception>
