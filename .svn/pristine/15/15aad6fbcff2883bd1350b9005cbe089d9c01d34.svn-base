...
namespace ClearCanvas.Server.ShredHost
{
    /// <summary>
    /// 
    /// 逻辑是程序可以在停止和运行之间停止，运行或转换，反之亦然。
    /// 在转换期间，不允许其他状态更改。例如，线程A尝试启动ShredHost:Start()
    /// 例程检查ShredHost是否处于停止状态，因为它没有意义开始运行ShredHost。
    /// 一旦检出，运行状态将更改为Transition。不应该有任何方法允许在转换期间继续更改状态。
    /// 一旦ShredHost启动，然后可以将运行状态设置为Running。
    /// 如果线程B试图在转换过程中阻止ShredHost 什么都不会发生。
    /// 如果线程B在运行时试图停止ShredHost，它将被停止成功。
    /// 
