...
namespace ClearCanvas.Dicom.Network
{
    ...
    /// <summary>DICOM服务器应用程序用于网络相关活动的类。</summary>
    public sealed class DicomServer : NetworkBase, IDisposable
    {
        ...
        /// </summary>
        /// <remarks>
        /// <para>请注意，可以使用不同的关联参数多次调用StartListening。
        /// Note that StartListening can be called multiple times with different association parameters.
        /// </para>
        /// </remarks>
        /// <param name="parameters">侦听关联时使用的参数。
        /// The parameters to use when listening for associations.</param>
        /// <param name="acceptor">要调用的委托，以返回实现的类实例<see cref ="IDicomServerHandler"/>接口处理传入的关联。
        /// A delegate to be called to return a class instance that implements
        /// the <see cref="IDicomServerHandler"/> interface to handle an incoming association.</param>
        /// 
        /// <returns>成功时<i> true </i>，失败时<i> false </i></returns>
        ...
        internal DicomServer(Socket socket, Dictionary<string, ListenerInfo> appList)
        {
            ...
            XTrace.WriteLine($"DicomServer 构造函数");
            XTrace.WriteLine($"remote = {remote.ToString()}");
            XTrace.WriteLine($"_host = {_host.ToJson()}");
            XTrace.WriteLine($"_socket = {_socket.ToString()}");
            XTrace.WriteLine($"_network = {_network.ToString()}");
            XTrace.WriteLine($"_closedEvent = {_closedEvent?.ToString()}");
            XTrace.WriteLine($"_handler = {_handler?.ToString()}");
            XTrace.WriteLine($"_appList = {_appList.ToJson()}");
        ...
        /// </summary>
		/// <param name="millisecondsTimeout">等待网络线程关闭的超时（以毫秒为单位）。
		/// The timeout in milliseconds to wait for the closure of the network thread.</param>
		protected override void CloseNetwork(int millisecondsTimeout)
        {

            XTrace.WriteLine($"关闭网络方法 CloseNetwork");
        ...
        protected override bool NetworkHasData()
        {
            ...
            //这是确定套接字是否仍处于活动状态的推荐方法零字节发送调用，并查看是否抛出异常。
            //请参阅Socket.Connected MSDN文档只有在我们知道没有可用数据时才进行检查.
            try
            {
        ...
        /// </summary>
        /// <param name="e">导致网络错误的异常
        /// The exception that caused the network error</param>
        /// <param name="closeConnection">标记告知是否应关闭连接
        /// Flag telling if the connection should be closed</param>
        protected override void OnNetworkError(Exception e, bool closeConnection)
        {
            XTrace.WriteLine($"调用网络错误的方法 OnNetworkError");
            ...
            catch (Exception x)
            {
                ...
                XTrace.WriteLine($"调用IDicomFilestreamHandler.CancelStream时出现意外异常");
                XTrace.WriteException(x);
            }
            ...
            catch (Exception x)
            {
                ...
                XTrace.WriteLine($"调用IDicomServerHandler.OnNetworkError时出现意外异常");
                XTrace.WriteException(x);
            }
        ...
        {
            XTrace.WriteLine($"接收关联请求时调用的方法 OnReceiveAssociateRequest： 参数{Environment.NewLine}" +
                             $"{association.ToDictionary().Join(Environment.NewLine)}");
            ...
            if (!_appList.TryGetValue(association.CalledAE, out info))
            {
                ...
                XTrace.WriteLine($"拒绝来自{association.CallingAE}的关联：无效的被叫AE Title（{association.CalledAE}）。");
            ...
            if (!anyValidContexts)
            {
                ...
                XTrace.WriteLine($"拒绝来自{association.CallingAE}的关联：没有有效的表示上下文。");
            ...
            catch (Exception e)
            {
                ...
                OnUserException(e, "Unexpected exception on OnReceiveAssociateRequest or StartDelegate" +
                                   "OnReceiveAssociateRequest或StartDelegate上出现意外异常");
            }
        }
