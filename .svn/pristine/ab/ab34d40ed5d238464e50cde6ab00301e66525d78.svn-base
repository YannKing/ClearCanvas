...
{
    /// <summary>用于表示DICOM上层状态机的内部枚举值（部分PS 3.8，第9.2.1节） 
    ...
    /// <summary>枚举值，表示标记中编码的优先级值<see cref ="DicomTags.Priority"/>。
    ...
    /// <summary>枚举值的枚举值<see cref ="DicomTags.CommandField"/>。
    ...
    public abstract class NetworkBase
    {
        ...
        /// <summary>在网络活动之后调用的方法，用于重置发生DIMSE超时的时间。
        ...
        protected void ResetDimseTimeout()
        {
            lock (_syncLock)
            {
                //TODO (Time Review): 使用Environment.TickCount。它可能不会有太多这里的性能影响，但在紧密循环中调用时效率非常低。
        ...
        /// <summary>返回Dimse Timeout的同步版本的时间
        ...
        /// </summary>
        /// <param name="millisecondsTimeout">等待网络关闭的超时（以毫秒为单位）。
        /// The timeout in milliseconds to wait for the closure of the network.</param>
        protected abstract void CloseNetwork(int millisecondsTimeout);

        /// <summary>用于将PDU排队以进行传输的内部例程。
        ...
        protected virtual void OnUserException(Exception e, String description)
        {
            ...
            XTrace.WriteLine("意外的用户异常，描述：" + description);
            XTrace.WriteException(e);

            XTrace.WriteLine($"OnUserException => State： {State.ToString()}");
            switch (State)
            {
                ...
                case DicomAssociationState.Sta6_AssociationEstablished:
                    ...
                    XTrace.WriteLine($"中止从{_assoc.CallingAE}到{_assoc.CalledAE}的关联");
        ...
        {
            throw new Exception("该方法或操作未实现。The method or operation is not implemented.");
        }
                ...
                while (!_stop)
                {
                    if (NetworkHasData())
                    {
                        ...
                        if (!success)
                        {
                            ...
                            // 启动Abort流程，我们无能为力
                            Platform.Log(LogLevel.Error, "Unexpected error processing PDU.  Aborting Association from {0} to {1}", _assoc.CallingAE, _assoc.CalledAE);

                            XTrace.WriteLine($"处理PDU时出现意外错误。从{_assoc.CallingAE}中止协会到{_assoc.CalledAE}");
                    ...
                    else if (DateTime.Now > GetDimseTimeout())
                    {
                        ...
                        switch (State)
                        {
                            ...
                            case DicomAssociationState.Sta2_TransportConnectionOpen:
                                ...
                                XTrace.WriteLine("等待AAssociate请求PDU时ARTIM超时，关闭连接。");
                            ...
                            case DicomAssociationState.Sta5_AwaitingAAssociationACOrReject:
                                ...
                                Platform.Log(LogLevel.Error, errorMessage);
                                XTrace.WriteLine($"等待AAssociate AC或RJ PDU时ARTIM超时，关闭连接。");
                            ...
                            case DicomAssociationState.Sta13_AwaitingTransportConnectionClose:
                                errorMessage = string.Format("Timeout when waiting for transport connection to close from {0} to {1}." +
                                                             " Dropping Connection.", _assoc.CallingAE, _assoc.CalledAE);
                                Platform.Log(LogLevel.Error, errorMessage);

                                XTrace.WriteLine($"等待传输连接从{_assoc.CallingAE}关闭到{_assoc.CalledAE}时的超时.Dropping Connection。");
                            ...
                            default:
                                ...
                                XTrace.WriteLine($"处于意外状态的DIMSE超时：{State.ToString()}");
        ...
        private bool ProcessRawPDU(RawPDU raw)
        {
            try
            {
                ...
                XTrace.WriteLine($"收到的PDU： {raw.ToString()}");
                switch (raw.Type)
                {
                    case 0x01:
                        {
                            ...
                            if (State != DicomAssociationState.Sta13_AwaitingTransportConnectionClose && State != DicomAssociationState.Sta6_AssociationEstablished)
                            {
                                Platform.Log(LogLevel.Error, "Association incorrectly not accepted or rejected, aborting.");
                                XTrace.WriteLine("协会错误地不被接受或拒绝，中止。");
                                return false;
                            }
                    ...
                    case 0xFF:
                        {
                            ...
                            XTrace.WriteLine($"意外的PDU类型：0xFF。潜在的解析错误。");
                            return false;
                        }
                    default:
                        throw new DicomNetworkException("Unknown PDU type 未知的PDU类型");
                }
            }
            catch (Exception e)
            {
        ...
        private bool ProcessPDataTF(PDataTFRead pdu)
        {
                ...
                foreach (PDV pdv in pdu.PDVs)
                {
                    ...
                    if (pdv.IsCommand)
                    {
                        ...
                        if (stat == DicomReadStatus.UnknownError)
                        {
                            ...
                            XTrace.WriteLine($"读取命令组元素时出现意外的解析错误。");
                            return false;
                        }
                        ...
                        if (pdv.IsLastFragment)
                        {
                            ...
                            {
                                Platform.Log(LogLevel.Error, "Unexpected end of StreamReader.  More data needed ({0} bytes, last tag read {1}) after reading last PDV fragment.", _dimse.CommandReader.BytesNeeded, _dimse.CommandReader.LastTagRead.ToString());

                                XTrace.WriteLine($"StreamReader的意外结束。在读取最后一个PDV片段之后需要更多数据（{_dimse.CommandReader.BytesNeeded}字节，最后一个标签读取{_dimse.CommandReader.LastTagRead.ToString()}）。");
                            ...
                            if (isLast)
                            {
                                ...
                                if (!ret)
                                {
                                    ...
                                    XTrace.WriteLine("OnReceiveDimse出错");
                                }
                    ...
                    else
                    {
                        ...
                        if (!_dimse.DatasetReader.EncounteredStopTag)
                        {
                            ...
                            if (_dimse.ParseDatasetStatus == DicomReadStatus.UnknownError)
                            {
                                ...
                                XTrace.WriteLine("读取DataSet时出现意外的解析错误。");
                                return false;
                            }
                        }
                        ...
                        if (_dimse.StreamMessage)
                        {
                            ...
                            if (!ret)
                            {
                                ...
                                XTrace.WriteLine("OnReceiveFileStream出错");
                            }
                        }
                        ...
                        if (pdv.IsLastFragment)
                        {
                            ...
                            {
                                Platform.Log(LogLevel.Error, "Unexpected end of StreamReader.  More data needed ({0} bytes, last tag read {1}) after reading last PDV fragment.", _dimse.DatasetReader.BytesNeeded, _dimse.DatasetReader.LastTagRead.ToString());

                                XTrace.WriteLine($"StreamReader的意外结束。在读取最后一个PDV片段后，需要更多数据（{_dimse.DatasetReader.BytesNeeded}字节，最后一个标签读取{_dimse.DatasetReader.LastTagRead.ToString()}）。");
                                return false;
                            }
                            _dimse.CommandData = null;
                            _dimse.CommandReader = null;
            ...
            catch (Exception e)
            {
                ...
                XTrace.WriteLine("意外异常处理P-DATA PDU");
                XTrace.WriteException(e);
                return false;
            }
        }
        ...
        private void SendDimse(byte pcid, DicomAttributeCollection command, DicomAttributeCollection dataset)
        {
            try
            {
                ...
                /*
                 为机票＃10147引入锁定作为风险缓解措施。注意更彻底的锁定
                 应该开发机制来跨PDU类型工作，也应该考虑到
                 如果我们最终使用multiThreaded = true
                 */
            ...
            catch (Exception e)
            {
                ...
                /* 
                 * TODO
                 * 我们应该在这里抛出另一个例外用户应该知道有错误吗？
                 * 他们会得到的通过OnNetworkError例程向他们报告错误，并在此处抛出异常可能会导致我们第二次调用OnNetworkError，
                 * 因为异常可能会被捕获更高水平.
                 * 注意，在修复缺陷＃8184时，意识到在这里抛出异常会导致ImageServer中的失败，
                 * 因为有些地方我们不会抓住它例外。如果再次介绍，应该小心。
                 * 抛出新的DicomException（“发送DIMSE消息时出现意外异常”，e）;
                 */
        ...
        private static void LogSendReceive(bool receive, DicomAttributeCollection metaInfo, DicomAttributeCollection dataSet)
        {
            if (Platform.IsLogLevelEnabled(LogLevel.Debug))
            {
                ...
                Platform.Log(LogLevel.Debug, receiveOrSend + " MetaInfo:\r\n" + (metaInfo != null ? metaInfo.DumpString : String.Empty));
                XTrace.WriteLine($"{receiveOrSend} MetaInfo:{Environment.NewLine} {(metaInfo != null ? metaInfo.DumpString : String.Empty)}");

                Platform.Log(LogLevel.Debug, receiveOrSend + " DataSet:\r\n" + (dataSet != null ? dataSet.DumpString : String.Empty));
                XTrace.WriteLine($"{receiveOrSend} DataSet:{Environment.NewLine} {(dataSet != null ? dataSet.DumpString : String.Empty)}");
