...
namespace ClearCanvas.ImageViewer.PresentationStates.Dicom
{
    ...
    /// DICOM Softcopy Presentation State对象的基类，如DICOM PS 3.3 A.33中所定义。
    /// </summary>
    /// <remarks>
    /// <para>
	...
	public abstract class DicomSoftcopyPresentationState : PresentationState
	{
		...
		public static DicomSoftcopyPresentationState Load(IDicomAttributeProvider presentationState)
		{
            ...
            else if (presentationState[DicomTags.SopClassUid].ToString() == DicomSecondarySoftcopyPresentationState.SopClass.Uid)//此处缺少二次捕获图像存储，使用灰度影像代替测试
            {
                //return new DicomGrayscaleSoftcopyPresentationState(presentationState);
                return new DicomSecondarySoftcopyPresentationState(presentationState);
            }
            else if (presentationState[DicomTags.SopClassUid].ToString() == DicomSecondarySoftcopyPresentationState.SopClassCT.Uid)//此处缺少二次捕获图像存储，使用灰度影像代替测试
            {
                //return new DicomGrayscaleSoftcopyPresentationState(presentationState);
                return new DicomSecondarySoftcopyPresentationState(presentationState);
            }
	...
	public enum DicomSoftcopyPresentationStateDeserializeOptions
	{
        ...
        /// 指定忽略存储的引用图像序列，并且呈现状态适用于所有请求的图像。
        /// </summary>
