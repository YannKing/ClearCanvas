...
//这将导致log4net查找配置文件
//在应用程序库中调用Logging.config
//目录（即包含TestApp.exe的目录）
//将监视配置文件的更改。

// 这将导致log4net查找配置文件在应用程序库中调用Logging.config目录（即包含TestApp.exe的目录）将监视配置文件的更改。
...
namespace ClearCanvas.Common
{
    ...
    public static class Platform
    {
        ...
        /// <summary>获取ClearCanvas安装目录。</summary>
        /// <remarks>此方法是线程安全的。</remarks>
                ...
                if (_pluginsDirectory == null)
                {
                    lock (_syncRoot)
                    {
                        if (_pluginsDirectory == null)
                        {
                            ...
                            // 如果PluginPath不存在，则回退到安装目录，以实现向后兼容性
        ...
        /// <summary>获取应用程序数据目录</summary>
        /// <returns></returns>
        private static string GetApplicationDataDirectory()
        {
            var path = Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData);

            //TODO 这似乎是从AssemblyCompanyAttribute派生的 //this seems to be derived from the AssemblyCompanyAttribute
            path = Path.Combine(path, "ClearCanvas_Inc");
            ...
            XTrace.WriteLine($"获取应用程序数据目录： {path}");
            return path;
        }
        ...
        /// <summary>
        /// 以<see cref ="TimeProviderExtensionPoint"/>的扩展名（如果存在）获取UTC的当前时间。
        /// 
        /// Gets the current time in UTC from an extension of <see cref="TimeProviderExtensionPoint"/>, if one exists.
        /// </summary>
        /// <remarks>
        /// <para>
        /// 返回的时间可能与此计算机上的当前时间不同，因为提供商可能会选择从另一个来源（即服务器）获得时间。
        /// 
        /// The time returned may differ from the current time on this machine, because the provider may choose
        /// to obtain the time from another source (i.e. a server).
        /// </para>
        ...
        private static DateTime GetCurrentTime(DateTimeKind kind)
        {
            ...
            //需要锁定此处，因为时间提供程序本身可能不是线程安全的
            //注意：锁定timeProvider而不是syncRoot，因此syncRoot对其他方法保持空闲
            lock (_timeProvider)
            {
        ...
        /// <param name="applicationRootFilter">一个扩展过滤器，用于选择要执行的应用程序根扩展。</param>
        /// <param name="args">从命令行传递的参数集。</param>
        /// <remarks>通过调用此方便方法启动基于ClearCanvas的应用程序某种类型的bootstrap可执行文件。
        /// 调用此方法会导致加载所有插件并创建<see cref ="IApplicationRoot"/>扩展名。
        /// 此方法不是线程安全的，因为它应该只在每次执行时由单个线程调用一次。
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// 通过调用此方便方法启动基于ClearCanvas的应用程序某种类型的bootstrap可执行文件。
        /// 调用此方法会导致加载所有插件和<see cref ="IApplicationRoot"/>扩展的创建。
        /// 此方法不是线程安全的，因为它应该只在每次执行时由单个线程调用一次。
        /// 
        ...
        /// <summary>启动与指定的完全或部分限定类名匹配的应用程序。</summary>
        /// <param name="appRootClassName">应用程序根类的名称，不需要完全限定。</param>
        /// <param name="args"></param>
        public static void StartApp(string appRootClassName, string[] args)
        {
            var methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
            //XTrace.WriteLine($"调用方法：{methodName} {Environment.NewLine}  " + $"{Environment.NewLine} ----END----");
            XTrace.WriteLine($"调用方法：{methodName} {Environment.NewLine}" + "" +
                             $"{Environment.NewLine} 参数： string appRootClassName： {appRootClassName}" +
                             $"{Environment.NewLine}  string[] args ：{args.Join(Environment.NewLine)}" +
                             $"{Environment.NewLine} ----END----");

            var appRoots = new ApplicationRootExtensionPoint().ListExtensions();

            XTrace.WriteLine($"调用方法：{methodName} {Environment.NewLine}" +
                             $"appRoots =>{appRoots.Join(Environment.NewLine)}" +
                             $"{Environment.NewLine} ----END----");

            //尝试完全匹配
            var matchingRoots = CollectionUtils.Select(appRoots, info => info.ExtensionClass.FullName == appRootClassName);
            XTrace.WriteLine($"调用方法：{methodName} {Environment.NewLine}  " +
                             $"matchingRoots 尝试完全匹配：{matchingRoots.Join(Environment.NewLine)}" +
                             $"{Environment.NewLine} ----END----");

            if (matchingRoots.Count == 0)
            {
                //尝试部分匹配
                matchingRoots = CollectionUtils.Select(appRoots, info => info.ExtensionClass.FullName.EndsWith(appRootClassName, StringComparison.InvariantCultureIgnoreCase));
                XTrace.WriteLine($"调用方法：{methodName} {Environment.NewLine}  " +
                                 $"matchingRoots 尝试部分匹配：{matchingRoots.Join(Environment.NewLine)}" +
                                 $"{Environment.NewLine} ----END----");
            }

            if (matchingRoots.Count == 0)
                throw new NotSupportedException(string.Format(SR.ExceptionApplicationRootNoMatches, appRootClassName));
            if (matchingRoots.Count > 1)
                throw new NotSupportedException(string.Format(SR.ExceptionApplicationRootMultipleMatches, appRootClassName));
        ...
        #region Service Provision 提供服务

        /// <summary>获取应用程序使用的指定服务的实例。</summary>
        /// <remarks>此方法是线程安全的。</remarks>
        /// <typeparam name="TService">要获得的服务类型。</typeparam>
        /// <returns>指定服务的实例。</returns>
        /// <exception cref="UnknownServiceException">无法提供所请求的服务。</exception>
        public static TService GetService<TService>()
        {
            return (TService)GetService(typeof(TService));
        }

        /// <summary>用于<see cref="GetService{TService}(WithServiceDelegate{TService})"/> 方法.</summary>
        public delegate void WithServiceDelegate<T>(T service);

        /// <summary>获取应用程序使用的指定服务的实例。</summary>
        /// <remarks>
        /// <para>此重载不是直接返回服务，而是将服务传递给指定的委托以供使用。
        /// 委托返回时，此方法自动负责确定服务是否实现<see cref ="IDisposable"/>
        /// 如果有的话，调用<see cref ="IDisposable.Dispose"/>。委托不得缓存返回的服务
        /// 因为代表返回后可能会立即处理。对于一次性使用场景，首选此重载
        /// 到其他重载，因为它自动管理服务对象的生命周期。</para>
        /// <para>此方法是线程安全的。</para>
        /// </remarks>
        /// <typeparam name="TService">获得的服务。</typeparam>
        /// <param name="proc">将接收一次性服务的代表。</param>
        public static void GetService<TService>(WithServiceDelegate<TService> proc)
        {
            ...
            finally
            {
                if (service is IDisposable)
                {
                    ...
                    {
                        //不允许从Dispose（）抛出的异常，因为它可能具有的效果隐藏从服务本身抛出的异常如果服务未能妥善处理，我们不在乎，只需记录并继续
        ...
        /// <summary>获取应用程序使用的指定服务的实例。
        /// Obtains an instance of the specified service for use by the application.  
        /// </summary>
        /// <remarks>
        /// <para>此重载不是直接返回服务，而是将服务传递给指定的委托以供使用。
        /// 委托返回时，此方法自动负责确定服务是否实现<see cref ="IDisposable"/>如果有的话，调用<see cref ="IDisposable.Dispose"/>。
        /// 委托不得缓存返回的服务因为代表返回后可能会立即处理。
        /// 对于一次性使用场景，首选此重载到其他重载，因为它自动管理服务对象的生命周期。
        /// 
        /// 
        ...
        /// </para>
        /// </remarks>
        /// <typeparam name="TService">获得的服务。
        /// The service to obtain.</typeparam>
        /// <typeparam name="TResult">函数结果的类型。
        /// The type of the function result.</typeparam>
        /// <param name="func">将接收一次性服务的代表。
        /// A delegate that will receive the service for one-time use.</param>
        ...
        /// This method is thread-safe.
        /// </remarks>
        /// <param name="service">要获得的服务类型。
        /// The type of service to obtain.</param>
        /// <returns>指定服务的实例。
        /// An instance of the specified service.</returns>
        /// <exception cref="UnknownServiceException">
        /// 无法提供所请求的服务。
        /// The requested service cannot be provided.</exception>
        public static object GetService(Type service)
        {
            ...
            {
                // the service provider itself may not be thread-safe, so we need to ensure only one thread will access it at a time
                // 服务提供者本身可能不是线程安全的，因此我们需要确保一次只有一个线程可以访问它
                lock (sp)
                {
        ...
        /// <summary>在指定的<see cref ="LogLevel"/>处记录指定的异常。</summary>
        /// <remarks>此方法是线程安全的。</remarks>
