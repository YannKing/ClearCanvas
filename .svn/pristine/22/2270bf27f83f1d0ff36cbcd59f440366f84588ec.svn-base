...
namespace ClearCanvas.ImageViewer.StudyManagement.Core.Storage
{
    ...
    {
        internal WorkItemBroker(DicomStoreDataContext context) : base(context)
        {
        ...
        private static readonly Func<DicomStoreDataContext, int, DateTime, IQueryable<WorkItem>> _getWorkItemsForProcessing = CompiledQuery.Compile<DicomStoreDataContext, int, DateTime, IQueryable<WorkItem>>(
            (context, n, now) => (from w in context.WorkItems
                                  where w.ProcessTime < now && (w.Status == WorkItemStatusEnum.Pending || w.Status == WorkItemStatusEnum.Idle)
                                  orderby w.Priority, w.ProcessTime
                                  select w)
            .Take(n));

        private static readonly Func<DicomStoreDataContext, int, DateTime, WorkItemPriorityEnum, IQueryable<WorkItem>> _getWorkItemsForProcessingByPriority = CompiledQuery.Compile<DicomStoreDataContext, int, DateTime, WorkItemPriorityEnum, IQueryable<WorkItem>>(
            (context, n, now, priority) => (from w in context.WorkItems
                                            where w.ProcessTime < now && w.Priority == priority && (w.Status == WorkItemStatusEnum.Pending || w.Status == WorkItemStatusEnum.Idle)
        ...
        private static readonly Func<DicomStoreDataContext, int, DateTime, IQueryable<WorkItem>> _getWorkItemsToDelete =
            CompiledQuery.Compile<DicomStoreDataContext, int, DateTime, IQueryable<WorkItem>>(
                (context, n, now) => (from w in context.WorkItems
        ...
        private static readonly Func<DicomStoreDataContext, int, IQueryable<WorkItem>> _getWorkItemsDeleted = CompiledQuery.Compile<DicomStoreDataContext, int, IQueryable<WorkItem>>(
            (context, n) => (from w in context.WorkItems
                             where w.Status == WorkItemStatusEnum.Deleted
                             select w)
            .Take(n));

        private static readonly Func<DicomStoreDataContext, long, IQueryable<WorkItem>> _getWorkItemByOid = CompiledQuery.Compile<DicomStoreDataContext, long, IQueryable<WorkItem>>((context, oid) => (from w in context.WorkItems where w.Oid == oid select w).Take(1));
