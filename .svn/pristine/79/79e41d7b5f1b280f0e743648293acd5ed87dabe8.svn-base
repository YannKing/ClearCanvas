...
namespace ClearCanvas.ImageViewer.Shreds.WorkItemService.ProcessStudy
{
    ...
    internal class StudyProcessProcessor : BaseItemProcessor<ProcessStudyRequest, ProcessStudyProgress>
    {
        ...
        public override void Delete()
        {
            ...
            foreach (WorkItemUid sop in WorkQueueUidList)
            {
                ...
                if (sop.Failed || !sop.Complete)
                {
                    if (!defaultFilePath)
                    {
                        ...
                        {
                            Platform.Log(LogLevel.Error, e, "Unexpected exception attempting to cleanup file for Work Item {0}", Proxy.Item.Oid);
                            XTrace.WriteLine($"尝试清除工作项{Proxy.Item.Oid}的文件时发生意外异常");
                    ...
                    else
                    {
                        try
                        {
                            ...
                            // 仅当文件不在研究Xml文件中时才删除该文件。这应该处理与碰撞多个WorkItem可能在其他人成功时被取消。
            ...
            {
                Platform.Log(LogLevel.Error, e, "Unexpected exception attempting to delete folder: {0}", Location.StudyFolder);
                XTrace.WriteLine($"尝试删除文件夹时出现意外异常：{Location.StudyFolder}");
                XTrace.WriteException(e);
            }

            // Now cleanup the actual WorkItemUid references 现在清理实际的WorkItemUid引用
        ...
        private bool ProcessUidList()
        {
            ...
            while (successfulProcessCount > lastSuccessProcessCount)
            {
                ...
                foreach (WorkItemUid sop in WorkQueueUidList)
                {
                    ...
                    if (CancelPending)
                    {
                        Platform.Log(LogLevel.Info, "Processing of study canceled: {0}", Location.Study.StudyInstanceUid);
                        XTrace.WriteLine($"Study处理已取消：{Location.Study.StudyInstanceUid}");
                    ...
                    if (StopPending)
                    {
                        ...
                        XTrace.WriteLine($"Study处理已停止：{ Location.Study.StudyInstanceUid}");
                        return successfulProcessCount > 0;
                    }
            ...
            catch (Exception e)
            {
                foreach (var sop in sops)
                {
                    try
                    {
                        ...
                        XTrace.WriteLine($"处理文件时出现意外异常：{path ?? string.Empty} SOP实例：{sop.SopInstanceUid}");
                        XTrace.WriteException(e);
                    ...
                    catch (Exception x)
                    {
                        ...
                        XTrace.WriteLine($"无法??失败WorkItemUid {sop.Oid}：{x.Message}");
