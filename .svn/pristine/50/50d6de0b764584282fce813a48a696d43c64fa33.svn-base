...
namespace ClearCanvas.ImageViewer.Common.Configuration
{
    public sealed class SystemConfigurationSettingsProvider : SettingsProvider, ISharedApplicationSettingsProvider
    {
        ...
        ///</summary>
        ///
        ///<param name="config">一组名称/值对，表示在此提供程序的配置中指定的特定于提供程序的属性。
        /// A collection of the name/value pairs representing the provider-specific attributes specified in the configuration for this provider.</param>
        ///<param name="name">提供者的友好名称。
        /// The friendly name of the provider.</param>
        public override void Initialize(string name, System.Collections.Specialized.NameValueCollection config)
        {
            lock (_syncLock)
            {
                // obtain a source provider 获得源提供商
                _store = Platform.GetService<ISystemConfigurationSettingsStore>();

                // init source provider， according to sample implementations, use the application name here
                //init源提供者,根据示例实现，请在此处使用应用程序名称
