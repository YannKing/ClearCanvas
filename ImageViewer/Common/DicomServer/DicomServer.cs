    ...
    public abstract class DicomServer : IDicomServer
    {
        static DicomServer()
        {
            ...
            catch (EndpointNotFoundException)
            {
                ...
                //这并不意味着它不受支持，这意味着它没有运行。
                IsSupported = true;
                XTrace.WriteLine("ExceptionType：EndpointNotFoundException ");
                XTrace.WriteLine("这并不意味着它不受支持，这意味着它没有运行。");
            }
            catch (NotSupportedException)
            {
                IsSupported = false;
                Platform.Log(LogLevel.Debug, "Local DICOM Server is not supported.");
                XTrace.WriteLine("不支持本地DICOM服务器。ExceptionType：NotSupportedException");
            }
            catch (UnknownServiceException)
            {
                IsSupported = false;
                Platform.Log(LogLevel.Debug, "Local DICOM Server is not supported.");
                XTrace.WriteLine("不支持本地DICOM服务器。ExceptionType：UnknownServiceException");
            }
            catch (Exception e)
            {
                IsSupported = false;
                Platform.Log(LogLevel.Debug, e, "Local DICOM Server is not supported.");
                XTrace.WriteLine("不支持本地DICOM服务器。");
            ...
            {
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        XTrace.WriteLine("无法联系本地DICOM服务器。");
                        XTrace.WriteException(e);
                        return false;
                    }
                }
        ...
        public static DicomServerConfiguration GetConfiguration()
        {
            ...
            Platform.GetService<IDicomServerConfiguration>(s => configuration = s.GetConfiguration(
                new GetDicomServerConfigurationRequest())
                .Configuration);
        ...
        {
            XTrace.WriteLine("DicomServer=>RestartListener()");
