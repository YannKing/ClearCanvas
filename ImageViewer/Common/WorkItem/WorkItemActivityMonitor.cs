...
namespace ClearCanvas.ImageViewer.Common.WorkItem
{
    ...
    public abstract partial class WorkItemActivityMonitor
    {
        static WorkItemActivityMonitor()
        {
            ...
            {
                //This doesn't mean it's not supported, it means it's not running.这并不意味着它不受支持，这意味着它没有运行。
                IsSupported = true;
            }
            catch (NotSupportedException)
            {
                ...
                XTrace.WriteLine($"Work Item Activity Monitor is not supported." +
                                  $"不支持工作项活动监视器。");
            }
    ...
    public abstract partial class WorkItemActivityMonitor : IWorkItemActivityMonitor
    {
        ...
        public static IWorkItemActivityMonitor Create(SynchronizationContext synchronizationContext)
        {
            ...
            lock (_instanceLock)
            {
                ...
                XTrace.WriteLine($"WorkItemActivityMonitor proxy created (count = { _proxyCount})" +
                                 $"创建了WorkItemActivityMonitor代理（count = {_proxyCount}）");
        ...
        internal static void OnProxyDisposed()
        {
            lock (_instanceLock)
            {
                ...
                XTrace.WriteLine($"WorkItemActivityMonitor proxy disposed (count = {_proxyCount})." +
                                 $"处理WorkItemActivityMonitor代理（count = {_proxyCount}）。");
                if (_proxyCount > 0)
                    return;

                var monitor = _instance;
                _instance = null;
                //No need to do this synchronously.无需同步执行此操作。
        ...
        public void Dispose()
        {
            ...
            catch (Exception e)
            {
                ...
                XTrace.WriteLine($"Unexpected error disposing WorkItemActivityMonitor." +
                                 $"处理WorkItemActivityMonitor时出现意外错误。");
