...
{
	[Cloneable]
	internal sealed class DicomSecondarySoftcopyPresentationState : DicomSoftcopyPresentationStateMaskLutBase<DicomSecondaryPresentationImage>
	{
        public static readonly SopClass SopClass = SopClass.SecondaryCaptureImageStorage;//.GrayscaleSoftcopyPresentationStateStorageSopClass;
        public static readonly SopClass SopClassCT = SopClass.CtImageStorage;//.GrayscaleSoftcopyPresentationStateStorageSopClass;
        public DicomSecondarySoftcopyPresentationState() : base(SopClass) {}

		public DicomSecondarySoftcopyPresentationState(DicomFile dicomFile) : base(SopClass, dicomFile) {}

		public DicomSecondarySoftcopyPresentationState(DicomAttributeCollection dataSource) : base(SopClass, dataSource) {}

		public DicomSecondarySoftcopyPresentationState(IDicomAttributeProvider dataSource) : base(SopClass, ShallowCopyDataSource(dataSource)) {}

		private DicomSecondarySoftcopyPresentationState(DicomSecondarySoftcopyPresentationState source, ICloningContext context) : base(source, context)
		{
        ...
        protected override void PerformTypeSpecificSerialization(DicomPresentationImageCollection<DicomSecondaryPresentationImage> images)
        {
            IOverlayMapping overlayMapping;
            SecondarySoftcopyPresentationStateIod iod = new SecondarySoftcopyPresentationStateIod(base.DataSet);
		...
		private void SerializeSoftcopyPresentationLut(SoftcopyPresentationLutModuleIod module, DicomPresentationImageCollection<DicomSecondaryPresentationImage> images)
		{
			var inverted = false;
			if (images.Count > 0)
			{
        ...
        protected override void PerformTypeSpecificDeserialization(DicomPresentationImageCollection<DicomSecondaryPresentationImage> images)
        {
            SecondarySoftcopyPresentationStateIod iod = new SecondarySoftcopyPresentationStateIod(base.DataSet);

            foreach (DicomSecondaryPresentationImage image in images)
            {
		...
		private void DeserializeSoftcopyPresentationLut(SoftcopyPresentationLutModuleIod module, DicomSecondaryPresentationImage image)
		{
