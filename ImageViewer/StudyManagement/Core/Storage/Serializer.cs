...
namespace ClearCanvas.ImageViewer.StudyManagement.Core.Storage
{
    ...
    {
        private static readonly DataContractSerializer _workItemRequestSerializer =
            CreateDataContractSerializer<WorkItemRequest, WorkItemRequestDataContractAttribute>();

        private static readonly DataContractSerializer _workItemProgressSerializer =
            CreateDataContractSerializer<WorkItemProgress, WorkItemProgressDataContractAttribute>();
        ...
        public static WorkItemRequest DeserializeWorkItemRequest(string data)
        {
            ...
            catch (Exception ex)
            {
                ...
                XTrace.WriteLine($"无法反序列化工作项请求，请使用旧版JSML格式重试");
                XTrace.WriteException(ex);
            }

            var res = JsmlSerializer.Deserialize<WorkItemRequest>(
                data,
                new JsmlSerializer.DeserializeOptions
                {
                    Hook = _workItemRequestHook,
                    DataContractTest = IsWorkItemRequestContract
                });
            return res;
        }
        ...
        public static WorkItemProgress DeserializeWorkItemProgress(string data)
        {
            ...
            catch (Exception ex)
            {
                ...
                XTrace.WriteLine($"无法反序列化工作项进度，请使用旧版JSML格式重试");
                XTrace.WriteException(ex);
            }

            var res = JsmlSerializer.Deserialize<WorkItemProgress>(
                data,
                new JsmlSerializer.DeserializeOptions
                {
                    Hook = _workItemProgressHook,
                    DataContractTest = IsWorkItemProgressContract
                });
            return res;
        }
        ...
        {
            return JsmlSerializer.Serialize(
                serverExtensionData,
                "data",
                new JsmlSerializer.SerializeOptions
                {
                    Hook = _serverExtensionDataHook,
                    DataContractTest = IsServerExtensionDataContract
                });
        }

        public static Dictionary<string, object> DeserializeServerExtensionData(string data)
        {
            return JsmlSerializer.Deserialize<Dictionary<string, object>>(
                data,
                new JsmlSerializer.DeserializeOptions
                {
                    Hook = _serverExtensionDataHook,
        ...
        private static DataContractSerializer CreateDataContractSerializer<TObject, TContractAttribute>() where TContractAttribute : PolymorphicDataContractAttribute
        {
