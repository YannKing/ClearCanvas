...
namespace ClearCanvas.ImageViewer.StudyManagement.Core.Configuration
{
	...
	internal class ConfigurationDocumentBroker : Broker
	{
        ...
        private static readonly Func<ConfigurationDataContext, string, string, string, IQueryable<ConfigurationDocument>> _queryPriorVersion =
			CompiledQuery.Compile<ConfigurationDataContext, string, string,  string, IQueryable<ConfigurationDocument>>(
				(context, documentName, instanceKey, paddedVersionString) =>
        ...
        private static readonly Func<ConfigurationDataContext, string, string,  string, IQueryable<ConfigurationDocument>> _queryCurrentVersion =
            CompiledQuery.Compile<ConfigurationDataContext, string, string, string,  IQueryable<ConfigurationDocument>>(
                (context, documentName, instanceKey, paddedVersionString) =>
                (from d in context.ConfigurationDocuments
                 where ((instanceKey == null && d.InstanceKey == null) || d.InstanceKey == instanceKey)
