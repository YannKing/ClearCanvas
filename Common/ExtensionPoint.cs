...
namespace ClearCanvas.Common
{
    ...
    public abstract class ExtensionPoint : IExtensionPoint
    {
        ...
        /// <summary>列出可用的扩展名。</summary>
        /// <returns>一个阵列 <see cref="ExtensionInfo" />描述可用扩展的对象。</returns>
        /// <remarks>可用扩展是指已启用和许可使用的扩展。</remarks>
        ...
        /// <summary>实际执行扩展创建的受保护方法来自内部<see cref ="IExtensionFactory"/>。 
        ...
        /// <summary>实际检索<see cref ="ExtensionInfo"/>的受保护方法 来自内部的对象<see cref ="IExtensionFactory"/>。
        /// </summary>
        protected ExtensionInfo[] ListExtensionsHelper(ExtensionFilter filter)
        {
            // we assume that the factory itself is thread-safe, and therefore we don't need to lock
            // (don't want to incur the cost of locking if not necessary)
            //我们假设工厂本身是线程安全的，因此我们不需要锁定
            //（如果没有必要，不要承担锁定费用）
